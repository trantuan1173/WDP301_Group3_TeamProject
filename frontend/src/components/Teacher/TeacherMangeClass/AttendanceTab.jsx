...
export default function AttendanceTab({ classId, students }) {
  ...
  if (!schedules.length) {
    return (
      <div className="w-full flex flex-col items-center justify-center min-h-[300px]">
        <h2 className="text-2xl font-bold mb-6 text-blue-900 text-center">Student List</h2>
  ...
  return (
    <div className="w-full flex flex-col items-start px-0">
      <h2 className="text-2xl font-bold mb-6 text-blue-900 text-left w-full pl-2">Student List</h2>
      <div className="flex flex-row gap-70 w-full justify-start items-start flex-wrap">
          ...
          <table className="min-w-full border rounded-lg shadow bg-white px-4">
            ...
            <tbody>
              {students && students.length > 0 ? students.map((student, idx) => (
                ...
                <tr>
                  <td colSpan={showAttendance && currentSchedule ? 5 : 3} className="text-center py-4">No students found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        {/* Attendance actions and info */}
       <div className="flex flex-col justify-between min-w-[220px] max-w-xs w-full h-full ml-8" style={{minHeight: 220}}>
          {!showAttendance ? (
            <button
              ...
              <div>
                {!currentSchedule && (
                  <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded text-yellow-800 font-semibold shadow mb-4 w-full text-center">
                    <div>It's not time for attendance yet.</div>
                    {nextSchedule ? (
                      <div className="mt-2 text-sm font-normal">
                        <div className="font-medium">Next schedule:</div>
                        <div>
                          Date: {new Date(nextSchedule.date).toLocaleDateString()}<br />
                          Time: {new Date(nextSchedule.start_time).toLocaleTimeString()} - {new Date(nextSchedule.end_time).toLocaleTimeString()}
                        </div>
                      </div>
                    ) : (
